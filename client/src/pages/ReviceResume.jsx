        ...
        <div className='h-full overflow-y-scroll p-6 flex items-start gap-8 flex-wrap text-[#1F1F1F]'>
            ...
            <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 border border-gray-200 bg-white rounded-lg' >
                ...
                <input className='w-full p-2 px-4 mt-2 outline-none border border-gray-300 text-sm rounded-md'
                    type="file" accept='application/pdf' placeholder='The future of artificial intelligence...' required value={input} onChange={(e) => setInput(e.target.files[0])} />

                <p className='mt-5 text-gray-500 text-xs font-light' > Supports PDF, PNG, JPG formats</p>
                <button type='submit' className='w-full flex items-center justify-center gap-2 mt-6 text-white px-4 py-2 rounded-lg text-sm cursor-pointer bg-gradient-to-r from-[#00DA83] to-[#009BB3]'>
            ...
            <div class="w-full max-w-lg sm:max-w-xl p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]">
                <div class="flex items-center gap-3">
                    <FileText class="w-5 h-5 text-[#00DA83" />
                    <h1 class="text-xl font-semibold">Analysis Results</h1>
                </div>
